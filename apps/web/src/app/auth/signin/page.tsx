      ...
      <section className="max-w-md mx-auto px-4 sm:px-6 pt-14 pb-20">
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="show" custom={2}
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            className="w-full flex items-center justify-center gap-3 rounded-xl border-2 border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 px-6 py-3.5 text-base font-semibold text-slate-700 dark:text-slate-200 hover:border-blue-300 hover:shadow-md transition-all">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            onClick={() => {
              if (selectedRole === 'provider') window.location.href = '/providers/apply'
              else if (selectedRole === 'admin') window.location.href = '/franchise/apply'
            }}
            className="w-full flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-6 py-3.5 text-base font-bold text-white shadow-lg shadow-blue-200 dark:shadow-blue-900/30 hover:bg-blue-700 transition-colors">
            {selectedRole === 'customer' ? '🛍️ เข้าสู่ระบบด้วย Google' : selectedRole === 'provider' ? '⭐ สมัครเป็น Provider →' : '🏘️ กรอกใบสมัครเปิดตลาด →'}
