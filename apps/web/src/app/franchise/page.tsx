...
const BENEFITS = [
  { icon: DollarSign, title: 'Revenue Share 40%', desc: 'รับ 40% จาก Commission ที่แพลตฟอร์มเก็บจากผู้ให้บริการในชุมชนของคุณ จ่ายทุกวันที่ 1 ของเดือน', color: 'text-green-600', bg: 'bg-green-50' },
  { icon: Users, title: 'จัดการชุมชนเอง', desc: 'อนุมัติ Provider รายใหม่ ตั้งราคา Promoted Listings ดูแลคุณภาพบริการ', color: 'text-blue-600', bg: 'bg-blue-50' },
  { icon: Shield, title: 'ทดลองใช้งาน ฿500/3 เดือน', desc: 'เริ่มต้นเพียง ฿500 สำหรับ 3 เดือนแรก จากนั้นเลือกแพ็คเกจที่เหมาะสมกับชุมชนของคุณ', color: 'text-purple-600', bg: 'bg-purple-50' },
  { icon: BarChart3, title: 'ข้อมูล Real-time', desc: 'Dashboard แสดงยอด Booking รายได้ และ Trust Score ของทุก Provider', color: 'text-amber-600', bg: 'bg-amber-50' },
  { icon: Zap, title: 'เครื่องมือครบครัน', desc: 'ระบบจัดการ Promotion, Notification ชุมชน และ Campaign Marketing', color: 'text-rose-600', bg: 'bg-rose-50' },
  { icon: Globe, title: 'แบรนด์ระดับชาติ', desc: 'อยู่ภายใต้แบรนด์ Community Hyper Marketplace ที่น่าเชื่อถือ', color: 'text-indigo-600', bg: 'bg-indigo-50' },
]
    ...
    <main className="min-h-screen overflow-x-hidden">
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-12 text-center">
        <motion.div variants={fadeUp} initial="hidden" animate="show" custom={0}>
          <span className="inline-flex items-center gap-2 text-sm font-bold text-amber-700 glass-sm border border-amber-200/60 px-4 py-1.5 rounded-full mb-5">
            <Building2 className="h-4 w-4" /> Community Franchise Program
          </span>
        </motion.div>
        <motion.h1 variants={fadeUp} initial="hidden" animate="show" custom={1}
          className="text-2xl sm:text-3xl font-extrabold text-slate-900 mb-4 leading-normal">
          เป็นเจ้าของ<span className="text-amber-500">ตลาดชุมชน</span><br />ในพื้นที่ของคุณ
        </motion.h1>
        <motion.p variants={fadeUp} initial="hidden" animate="show" custom={2}
          className="text-sm sm:text-base text-slate-500 mb-8 max-w-2xl mx-auto leading-relaxed">
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="show" custom={3}
          ...
          <Link href="#pricing" className="text-base font-semibold text-slate-600 hover:text-amber-600 transition-colors">
            ดูแพ็คเกจราคา →
          </Link>
        </motion.div>
      </section>
        ...
        <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true }}
          ...
          {[
            ...
            <motion.div key={s.label} variants={fadeUp} custom={i}
              ...
              <div className="text-sm text-primary font-bold">{s.suffix}</div>
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        <motion.h2 variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }}
          className="text-3xl font-extrabold text-slate-900 text-center mb-8">
          สิ่งที่คุณได้รับ
        </motion.h2>
        <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true }}
          ...
          {BENEFITS.map((b, i) => (
            <motion.div key={b.title} variants={fadeUp} custom={i} whileHover={{ y: -4 }}
              ...
              <h3 className="font-extrabold text-base text-slate-800 mb-1">{b.title}</h3>
      ...
      <section id="revenue" className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        <motion.h2 variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }}
          className="text-3xl font-extrabold text-slate-900 text-center mb-2">
          รายได้ที่คุณจะได้รับ
        </motion.h2>
        <motion.p variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} custom={1}
          className="text-center text-slate-500 mb-8 text-base leading-relaxed">
        ...
        <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} custom={2}
          ...
          {REVENUE_MODEL.map((row, i) => (
            <div key={i} className={`flex items-center justify-between px-6 py-4 ${
              row.highlight ? 'glass-sm border-t-2 border-green-200' : 'border-b border-white/20'
            }`}>
              <div>
                <p className={`text-base font-semibold ${row.highlight ? 'text-green-800' : 'text-slate-700'}`}>{row.label}</p>
                {row.note && <p className="text-sm text-slate-400">{row.note}</p>}
              </div>
              <p className={`text-xl font-extrabold ${row.highlight ? 'text-green-600' : 'text-slate-800'}`}>{row.example}</p>
            </div>
          ))}
        </motion.div>
        <p className="text-center text-sm text-slate-400 mt-3">* ตัวเลขเป็นการประมาณการ ขึ้นอยู่กับขนาดและกิจกรรมในชุมชน</p>
      </section>
        ...
        <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true }}
          ...
          {STEPS_PROCESS.map((s, i) => (
            <motion.div key={s.step} variants={fadeUp} custom={i}
              ...
              <div className="text-4xl font-extrabold text-primary/20 mb-3">{s.step}</div>
              <h3 className="font-extrabold text-base text-slate-800 mb-1">{s.title}</h3>
      ...
      <section id="pricing" className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} className="text-center mb-3">
          <span className="inline-flex items-center gap-2 text-sm font-bold text-amber-700 glass-sm border border-amber-200/60 px-4 py-1.5 rounded-full mb-4">
        ...
        <motion.p variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} custom={2}
          className="text-center text-slate-500 mb-10 text-base">
          ...
          {PRICING_PLANS.map((plan, i) => (
            <motion.div key={plan.id} variants={fadeUp} custom={i} whileHover={{ y: -6 }}
              className={`relative glass-card rounded-2xl p-6 flex flex-col ${
                plan.highlight
                  ? 'border-amber-400 shadow-amber-200/50 ring-2 ring-amber-400'
                  : ''
              }`}>
              {plan.badge && (
              ...
              <div className="mb-4">
                <p className="text-sm font-bold text-slate-500 mb-1">{plan.label}</p>
              ...
              <Link href="/franchise/apply"
                className={`flex items-center justify-center gap-2 rounded-xl py-3 text-sm font-bold transition-colors ${
                  plan.highlight
                    ...
                    : 'glass-sm text-slate-700 hover:border-white/40'
                }`}>
          ...
          {ACTIVE_COMMUNITIES.map((c, i) => (
            <motion.div key={c.name} variants={fadeUp} custom={i} whileHover={{ y: -4 }}
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm text-slate-500">{c.province}</span>
              </div>
              <h3 className="font-extrabold text-base text-slate-800 mb-3">{c.name}</h3>
              <div className="grid grid-cols-3 gap-2 text-center">
                <div><p className="text-xl font-extrabold text-slate-900">{c.providers}</p><p className="text-xs text-slate-400">Provider</p></div>
                <div><p className="text-xl font-extrabold text-amber-500">{c.rating}</p><p className="text-xs text-slate-400">⭐ Rating</p></div>
                <div><p className="text-xl font-extrabold text-slate-900">{c.bookings.toLocaleString()}</p><p className="text-xs text-slate-400">Booking</p></div>
              </div>
            </motion.div>
          ))}
        </motion.div>
